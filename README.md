...
### Project contents 777
The microservice application has a health endpoint which is accessible at `<host>:<port>/demoxx/health`. The context root is set in the `src/main/webapp/WEB-INF/ibm-web-ext.xml` file. The ports are set in the pom.xml file and exposed to the CLI in the cli-config.yml file.
